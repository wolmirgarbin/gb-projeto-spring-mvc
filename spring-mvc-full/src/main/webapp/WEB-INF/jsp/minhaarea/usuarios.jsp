		...
		<table id="table-usuarios">
			...
			<tbody class="custom-usuarios">
				<c:forEach var="pessoa" items="${ listPessoas }">
					<tr class="line border ${ empty pessoa.email ? 'active' : '' }">
						<td class="nome"><span>${ pessoa.nome }</span></td>
						<td><span>${ pessoa.identificacao }</span></td>
						<td><input class="save-email" type="text" data-id="${ pessoa.id }" value="${ not empty pessoa.email ? pessoa.email : '' }"></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
