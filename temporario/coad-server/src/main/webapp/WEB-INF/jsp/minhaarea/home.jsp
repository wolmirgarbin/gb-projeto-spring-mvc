...
<div class="container">
	...
	<div class="formulario for-minha-area clearfix">
		...
		<input type="hidden" value="${ tipoBusca }" id="tipoBusca">
	
		<h3>Filtrar arquivos ${ tipoBusca.nomeFiltro } por:</h3>
		<div class="field">
		...
		<c:if test="${ isAdm || isSup }">
			...
			<div class="field">
				<label for="dataInicialCarregamento">Data de carregamento</label>
				<input class="date" type="text" id="dataInicialCarregamento" maxlength="10">
				<input class="date" type="text" id="dataFinalCarregamento" maxlength="10">
			</div>
			<div class="field">
				<label for="status">Status</label>
				<input class="g-menor numeric" type="text" id="status" maxlength="8">
			</div>
			<div class="field">
				<label for="nome">Nome ${ tipoBusca.labelField }</label>
		...
		<div class="barra-campo">
			...
			<label for="ordenacao" class="separa-ordenacao">Ordenar os registros por</label>
			<select id="ordenacao" style="width: 230px;">
				<c:forEach items="${ lsOrdenacao }" var="item">
					<option value="${ item }" ${ item.padrao ? 'selected' : '' }>${ item.label }</option>
				</c:forEach>
			</select>
		</div>
		...
		<table id="table-list">
			<thead>
				<tr>
					...
					<th>
						<span class="text">${ tipoBusca.labelField }</span>
					</th>
					<th width="105px">
