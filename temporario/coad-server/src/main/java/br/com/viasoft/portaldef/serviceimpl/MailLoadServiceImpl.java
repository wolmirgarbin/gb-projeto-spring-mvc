...
public class MailLoadServiceImpl extends AbstractMail implements MailLoadService {
	...
	public void loadMail() {
    	try {
    		...
    		// atualiza a data e hora e para não ler todas as mensagens
    		configRepositoryCustom.updateDtHrLeitura(config);
			...
			// Get a Store object
			Store store = session.getStore("imaps");	//I change it from imap to imaps

			// Connect
			store.connect( HOST, applicationProperties.getLoadEmail(), applicationProperties.getLoadSenha());
			...
			for (int i = 0; i < count; i++) {
				try {
					//if( ! messages[i].getFlags().equals( Flags.Flag.SEEN ) ) {

						// faz a leitura dos anexos
		    			final List<FilePatternTO> anexos = loadAnexo(messages[i]);

		    			// adiciona os arquivos na lista prioncipal
    ...
    @Override
	public List<FilePatternTO> loadAnexo(final Message msg){
    	...
    	try {
    		...
    		if (content instanceof Multipart) {
	            ...
	            for (int i = 0; i < count; i++) {
	            	...
	            	if( part.getDisposition() != null && (part.getDisposition().equalsIgnoreCase(BodyPart.ATTACHMENT)) ) {
     ...
     * @return
     */
    private List<FilePatternTO> ajustaNomePDF(List<FilePatternTO> grupoAnexo) {
    	// se tem dois arquivos neste email
    	if( grupoAnexo.size() == 2 ) {
    		// se um for xml e o outro pdf
    		FilePatternTO filePatternXml = null, filePatternPdf = null;
    		for (final FilePatternTO filePatternTO : grupoAnexo) {
    			if( filePatternTO.getTipo().equals("xml") ){
    				filePatternXml = filePatternTO;
    			} else if( filePatternTO.getTipo().equals("pdf") ){
    				filePatternPdf = filePatternTO;
    			}
			}
    		if( filePatternXml != null && filePatternPdf != null ) {

    			// pega a chave do arquivo xml
    			final String chave = TipoDaNotaUtil.verificaNotaRetornaChave(filePatternXml);

    			if( chave != null ) {
	    			for (final FilePatternTO filePatternTO : grupoAnexo) {
	    				// renomear o arquivo pdf com o id do arquivo do xml
	    				if( filePatternTO.getTipo().equals("pdf") ){
	        				if( ! filePatternTO.getNome().contains(chave) ){
	        					filePatternTO.setNome( "DANFE-"+ chave +".pdf" );
	        				}
	        			}
	    			}
    			}
