...
$(document).ready(function(){
	...
	if( $dataTableHome.size() > 0 ) {
		...
		$("#qtdRegistros, #ordenacao").change(function(){
			atualizaLista(true);
		});
	...
	// tela de configuração do portal, mostrar ou ocultar senha
	var $ocultarSenhaEmailLeitura = $("#ocultarSenhaEmailLeitura");
	$ocultarSenhaEmailLeitura.click(function(){
		if( $ocultarSenhaEmailLeitura.is(":checked") ){
			$("#loadSenha").attr("type","text");
		} else {
			$("#loadSenha").attr("type","password");
		}
	});
	
	
	// help nas paginas do sistema
	var $opemHelp = $("#opemHelp");
	$opemHelp.click(function(){
		var $opemHelpUl = $("#opemHelpUl");
		if( $opemHelpUl.is(":visible") ) {
			$opemHelpUl.slideUp();
		} else {
			$opemHelpUl.slideDown();
		}
	});
...
function atualizaLista(clear) {
	...
	filter.dataInicialCarregamento = $("#dataInicialCarregamento").val();
	filter.dataFinalCarregamento = $("#dataFinalCarregamento").val();
	...
	filter.ordenacao = $("#ordenacao").val();
...
function template(json) {
	...
	if( json.notas.length == 0 ) {
		...
		for( i in json.notas ) {
			...
			html += 			"<span class='title'>Download XML</span>";
			...
			if( nota.idDanf != null ) {
				html += 		"<div class='parte'>";
				html += 			"<span class='title'>Download PDF</span>";
